/*
  Dictionnaire FRANCAIS. Source unique de tous les textes visibles du site.

  Regles de tenue de ce fichier :
  - Ce fichier definit la FORME du dictionnaire. en.ts est type dessus
    (`satisfies typeof fr`), donc toute cle ajoutee ici doit etre ajoutee
    dans en.ts, sinon le build echoue. C'est voulu : impossible d'oublier
    une traduction.
  - On ne met ici que du TEXTE. La configuration (geometrie SVG, drapeaux
    d'affichage, cles d'API, chemins d'images) reste dans les composants.
  - Les tableaux sont fusionnes PAR INDEX avec la configuration des
    composants (offres, temoignages) : ne jamais changer l'ordre sans
    verifier le composant correspondant.
*/
